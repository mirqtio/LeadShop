...
from .utils import (
    ...
    sync_get_lead_info,
    sync_assess_google_business_profile,
    run_async_in_celery,
    debug_async_function_call,
    prepare_assessment_data_for_storage,
    ASSESSMENT_STATUS,
    AssessmentError
)
...
def pagespeed_task(self, lead_id: int) -> Dict[str, Any]:
    ...
    try:
        ...
        # Get lead information using sync function
        url, company = sync_get_lead_info(lead_id)
        if not url:
            ...
            if pagespeed_results.get("cost_records"):
                ...
                with SyncSessionLocal() as session:
                    try:
                        ...
                        session.commit()
                        logger.debug(f"Stored {len(pagespeed_results['cost_records'])} cost records for lead {lead_id}")
                    except Exception as cost_exc:
                        session.rollback()
                        logger.error(f"Failed to store cost records for lead {lead_id}: {cost_exc}")
        ...
        # Prepare data for storage by converting AssessmentCost objects to serializable format
        pagespeed_data_for_storage = prepare_assessment_data_for_storage(pagespeed_results)
        
        # Store results in database using sync functions
        ...
        # Store error in database using sync functions
        try:
            sync_update_assessment_field(
                lead_id,
                'pagespeed_data',
            ...
            sync_update_assessment_field(lead_id, 'pagespeed_score', 0)
...
def security_task(self, lead_id: int) -> Dict[str, Any]:
    ...
    try:
        ...
        try:
            ...
            technical_results = run_async_in_celery(assess_technical_security, url, company, lead_id)
            ...
            if technical_results.cost_records:
                try:
                    ...
                    logger.warning(f"Failed to store cost records for lead {lead_id}: {cost_exc}")
        ...
        sync_update_assessment_field(lead_id, 'security_headers', technical_data)
        sync_update_assessment_field(lead_id, 'security_score', final_security_score)
        ...
        try:
            ...
            sync_update_assessment_field(lead_id, 'security_score', 0)
        except Exception as db_exc:
...
def gbp_task(self, lead_id: int) -> Dict[str, Any]:
    ...
    try:
        ...
        # Get lead information using sync function
        company, address, city, state = sync_get_lead_info(lead_id, fields=['company', 'address', 'city', 'state'])
        ...
        try:
            # Execute GBP assessment with fuzzy matching and cost tracking using sync wrapper
            gbp_results = sync_assess_google_business_profile(
        ...
        # Prepare data for storage by converting AssessmentCost objects to serializable format
        gbp_data_for_storage = prepare_assessment_data_for_storage(gbp_results)
        
        # Store results in database using sync function
        sync_update_assessment_field(lead_id, 'gbp_data', gbp_data_for_storage)
        ...
        try:
            ...
            sync_update_assessment_field(lead_id, 'mobile_score', 0)
        except Exception as db_exc:
...
def screenshot_task(self, lead_id: int) -> Dict[str, Any]:
    ...
    try:
        ...
        try:
            ...
            screenshot_results = run_async_in_celery(capture_website_screenshots, url, lead_id)
        ...
        # Prepare data for storage by converting AssessmentCost objects to serializable format
        screenshot_data_for_storage = prepare_assessment_data_for_storage(screenshot_data)
        
        # Store results in database
        sync_update_assessment_field(
            lead_id,
            'visual_analysis',
            screenshot_data_for_storage
...
def semrush_task(self, lead_id: int) -> Dict[str, Any]:
    ...
    try:
        ...
        try:
            # Execute SEMrush domain analysis with proper async handling
            semrush_results = run_async_in_celery(assess_semrush_domain, domain, lead_id)
            ...
            # Store cost records in database if available
        ...
        # Prepare data for storage by converting AssessmentCost objects to serializable format
        semrush_data_for_storage = prepare_assessment_data_for_storage(semrush_data)
        
        # Store results in database
        sync_update_assessment_field(lead_id, 'semrush_data', semrush_data_for_storage)
        sync_update_assessment_field(lead_id, 'seo_score', seo_score)
        ...
        try:
            ...
            sync_update_assessment_field(lead_id, 'seo_score', 0)
        except Exception as db_exc:
...
def visual_task(self, lead_id: int) -> Dict[str, Any]:
    ...
    try:
        ...
        # Get screenshot URLs from assessment data
        desktop_url = None
        mobile_url = None
        
        # Get assessment data to check for existing screenshots
        try:
            assessment_data, = sync_get_lead_info(lead_id, fields=['assessment_data'])
            if assessment_data and assessment_data.get('visual_analysis'):
                visual_data = assessment_data['visual_analysis']
                if isinstance(visual_data, dict):
                    ...
                    if desktop_data.get("s3_url"):
                        desktop_url = desktop_data["s3_url"]
                    if mobile_data.get("s3_url"):
                        mobile_url = mobile_data["s3_url"]
        except Exception as e:
            logger.warning(f"Could not retrieve assessment data for lead {lead_id}: {e}")
            # Continue without existing screenshot URLs
        
        if not desktop_url or not mobile_url:
        ...
        try:
            ...
            visual_results = run_async_in_celery(assess_visual_analysis, url, desktop_url, mobile_url, lead_id)
            ...
            # Prepare data for storage by converting AssessmentCost objects to serializable format
            visual_data_for_storage = prepare_assessment_data_for_storage(visual_data)
            
            # Update assessment record with visual analysis results
            sync_update_assessment_field(
                str(lead_id),
                'visual_analysis',
                visual_data_for_storage
            )
            # Visual score is included in the visual_data dict above - no separate field needed
            
            # Store cost records in database (using sync session)
            if visual_results.cost_records:
                try:
                    ...
                    logger.warning(f"Failed to store cost records for lead {lead_id}: {e}")
        ...
        sync_update_assessment_status(str(lead_id), ASSESSMENT_STATUS.VISUAL_COMPLETED)
...
def score_calculation_task(self, lead_id: int) -> Dict[str, Any]:
    ...
    try:
        ...
        lead_data, assessment_data = run_async_in_celery(get_assessment_data)
        ...
        try:
            ...
            business_score = run_async_in_celery(calculate_business_score, lead_id, assessment_data, lead_data)
            
            # Extract priority recommendations from all components
            priority_recommendations = []
            # Get all available components, handle missing visual_score gracefully
            components = [business_score.performance_score, business_score.security_score, 
                         business_score.seo_score, business_score.ux_score]
            if hasattr(business_score, 'visual_score'):
                components.append(business_score.visual_score)
            ...
            score_data = {
                ...
                "component_scores": {
                    ...
                    "visual": {
                        "raw_score": business_score.visual_score.raw_score if hasattr(business_score, 'visual_score') else 0,
                        "impact_estimate": business_score.visual_score.impact_estimate if hasattr(business_score, 'visual_score') else 0,
                        "severity": business_score.visual_score.severity if hasattr(business_score, 'visual_score') else "P3",
                        "recommendations": business_score.visual_score.recommendations if hasattr(business_score, 'visual_score') else []
                    }
                },
                "business_factors": {
        ...
        sync_update_assessment_status(lead_id, ASSESSMENT_STATUS.SCORING_COMPLETED)
        
        # Count critical and high priority issues
        score_components = [business_score.performance_score, business_score.security_score, 
                           business_score.seo_score, business_score.ux_score]
        if hasattr(business_score, 'visual_score'):
            score_components.append(business_score.visual_score)
            
        critical_issues = sum(1 for score in score_components if score.severity == 'P1')
        high_priority_issues = sum(1 for score in score_components if score.severity == 'P2')
        ...
        try:
            ...
            sync_update_assessment_field(lead_id, 'overall_score', 0)
        except Exception as db_exc:
...
def content_generation_task(self, lead_id: int) -> Dict[str, Any]:
    ...
    try:
        ...
        business_data, assessment_data = run_async_in_celery(get_content_data)
        ...
        try:
            ...
            marketing_content = run_async_in_celery(generate_marketing_content, lead_id, business_data, assessment_data)
        ...
        sync_update_assessment_status(lead_id, ASSESSMENT_STATUS.CONTENT_COMPLETED)
...
def full_assessment_orchestrator_task(self, lead_id: Optional[int] = None, lead_data: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    try:
        ...
        try:
            ...
            execution_result = run_async_in_celery(execute_full_assessment, lead_id, lead_data)
...
def report_generation_task(self, lead_id: int) -> Dict[str, Any]:
    ...
    try:
        ...
        lead = run_async_in_celery(get_lead_data)
        ...
        try:
            ...
            report_result = run_async_in_celery(generate_assessment_report, lead_id)
...
def email_formatting_task(self, lead_id: int) -> Dict[str, Any]:
    ...
    try:
        ...
        content_data = run_async_in_celery(get_content_data)
        ...
        try:
            ...
            formatted_email = run_async_in_celery(format_business_email, lead_id, content_data)
...
def testing_dashboard_task(self, lead_id: int) -> Dict[str, Any]:
    ...
    try:
        ...
        try:
            ...
            test_result = run_async_in_celery(run_full_pipeline_test, lead_data, user_id=1)
