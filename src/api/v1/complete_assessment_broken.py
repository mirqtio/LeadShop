"""
Complete Assessment API - All 8 Components
"""
import asyncio
import logging
...
async def run_complete_assessment(lead_id: int, assessment_id: int):
    ...
    try:
        # Get lead and assessment data first
        url = None
        business_name = None
        ...
        try:
            ...
            try:
                ...
                if desktop_url and mobile_url:
                    results["visual"] = await asyncio.wait_for(
                        assess_visual_analysis(url, desktop_url, mobile_url, lead_id, assessment_id),
                        timeout=30
                    )
            ...
            # Save to database - need a new session
            async with AsyncSessionLocal() as db2:
                # Get the assessment again in this session
                assessment_to_update = await db2.get(Assessment, assessment_id)
                if assessment_to_update:
                    assessment_to_update.pagespeed_data = results.get("pagespeed")
                    assessment_to_update.security_headers = results.get("security")
                    assessment_to_update.semrush_data = results.get("semrush")
                    assessment_to_update.gbp_data = results.get("gbp")
                    
                    # Handle screenshots - convert Pydantic model to dict if needed
                    screenshots_data = results.get("screenshots")
                    if screenshots_data:
                        if hasattr(screenshots_data, 'dict'):
                            assessment_to_update.screenshots = screenshots_data.dict()
                        elif hasattr(screenshots_data, 'model_dump'):
                            assessment_to_update.screenshots = screenshots_data.model_dump()
                        else:
                            assessment_to_update.screenshots = screenshots_data
                    else:
                        assessment_to_update.screenshots = []  # Use empty list instead of None
                    
                    # Handle visual analysis - convert Pydantic model to dict if needed
                    visual_data = results.get("visual")
                    if visual_data:
                        if hasattr(visual_data, 'dict'):
                            assessment_to_update.visual_analysis = visual_data.dict()
                        elif hasattr(visual_data, 'model_dump'):
                            assessment_to_update.visual_analysis = visual_data.model_dump()
                        else:
                            assessment_to_update.visual_analysis = visual_data
                    else:
                        assessment_to_update.visual_analysis = None
                        
                    assessment_to_update.total_score = overall_score
                    assessment_to_update.content_generation = content
...
async def complete_assessment_ui():
    ...
    return open('/app/static/complete_assessment_ui.html', 'r').read()
...
async def get_status(assessment_id: int):
    ...
    if assessment_id in assessment_status:
        status_data = assessment_status[assessment_id]
        ...
        if status_data.get("status") in ["completed", "failed"]:
            async with AsyncSessionLocal() as db:
                ...
                if assessment:
                    ...
                    # Access all fields while in async context
                    status_data["db_results"] = {
                        "pagespeed": assessment.pagespeed_data if assessment.pagespeed_data else None,
                        "security": assessment.security_headers if assessment.security_headers else None,
                        "semrush": assessment.semrush_data if assessment.semrush_data else None,
                        "gbp": assessment.gbp_data if assessment.gbp_data else None,
                        "screenshots": assessment.screenshots if assessment.screenshots else None,
                        "visual": assessment.visual_analysis if assessment.visual_analysis else None,
                        "content": assessment.content_generation if assessment.content_generation else None
                    }
        ...
        async with AsyncSessionLocal() as db:
            ...
            if assessment and assessment.total_score is not None:
                return {
                    ...
                    "database_saved": True,
                    "db_score": assessment.total_score,
                    "results": {
                        "pagespeed": assessment.pagespeed_data if assessment.pagespeed_data else None,
                        "security": assessment.security_headers if assessment.security_headers else None,
                        "semrush": assessment.semrush_data if assessment.semrush_data else None,
                    ...
                    "components": {
                        "pagespeed": "completed" if assessment.pagespeed_data else "failed",
                        "security": "completed" if assessment.security_headers else "failed",
                        "semrush": "completed" if assessment.semrush_data else "failed",
                        "gbp": "completed" if assessment.gbp_data else "failed",
                        "screenshots": "completed" if assessment.screenshots else "failed",
                        "visual": "completed" if assessment.visual_analysis else "failed",
                        "score": "completed" if assessment.total_score is not None else "failed",
                        "content": "completed" if assessment.content_generation else "failed"
                    }
                }
